...
test('manifest packs allow brand briefs and README metadata alongside images',async()=>{const crops=Array.from({length:5},(_,i)=>({position:i+1,x:i*.2,y:0,width:.2,height:1})),manifest={schemaVersion:1,id:'branded-system',version:'1.0.0',templates:[{id:'direction-one',name:'Direction one',mode:'board',image:'templates/board.png',crops}]};const bytes=zip([['pack.json',JSON.stringify(manifest)],['brand-brief.json',JSON.stringify({primary:'#123456'})],['README.txt','Import instructions'],['templates/board.png',png],['logo-original',png]]);const staged=await importer.stageTemplateImport({clientId:client.id,businessPackId:'salon',scope:'client',bytes});assert.equal(staged.preview.kind,'manifest');assert.equal(staged.preview.images.length,2);assert.equal(staged.preview.unresolved,false)});
