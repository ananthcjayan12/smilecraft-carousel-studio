...
export async function runTextProvider(task, payload) {
  ...
  try {
    if (provider === 'openai') {
      ...
      const result = normalizeTextResult(task, parsed, label, payload.contextSnapshot?.recipe?.roles);
    ...
    const parsed = parseJsonResponse(raw, label); const result = normalizeTextResult(task, parsed, label, payload.contextSnapshot?.recipe?.roles);
